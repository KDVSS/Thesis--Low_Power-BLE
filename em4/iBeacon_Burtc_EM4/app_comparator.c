...
#define COMP_VPOS_PORT            gpioPortB
#define COMP_VPOS_PIN             4
#define INTERRUPT_PORT            gpioPortB
...
void handle_super_capacitor_voltage(double superCapVoltage)
{
  ...
  if (sleep_in_EM4 && superCapVoltage >= Vmax) {
      ...
      DEBUG_PRINT_MAX("Super Capacitor is not fully charged(%0.2fV), Enter_EM4 -> "
             ...
             GPIO_PinOutGet(COMP_VPOS_PORT, COMP_VPOS_PIN));
...
void disableBURTCInterrupt(void)
{
    // Disable the compare interrupt
    BURTC_IntDisable(BURTC_IEN_COMP);

    // Optionally clear any pending interrupt flag
    BURTC_IntClear(BURTC_IF_COMP);

    // Optionally disable the BURTC itself if no longer needed
    BURTC_Enable(false);

    printf("-- Disabled BURTC --\r\n");
}
...
{
  unsigned int intNo = 4; //PB03 is mapped to External Wake-Up Source -> EM4WU4
  GPIO_PinModeSet(INTERRUPT_PORT, INTERRUPT_PIN, gpioModeInput, 0); // Pull-up enabled

  // Configure EM4 wake-up interrupt for rising edge on PB03 (EM4WU4), when (V_OUT goes high)
  uint32_t polarityMask = (uint32_t)false << (intNo + _GPIO_EM4WUEN_EM4WUEN_SHIFT);
  GPIO_EM4EnablePinWakeup(GPIO_IEN_EM4WUIEN4, polarityMask);
  GPIO_ExtIntConfig(INTERRUPT_PORT, INTERRUPT_PIN, intNo, true, false, true);
  //GPIO_EM4WUExtIntConfig(INTERRUPT_PORT, INTERRUPT_PIN, 4, false, true);

  printf("-- Configured GPIO Interrupt --\r\n");
}

void disable_GPIO_EM4Wakeup(void) {
    printf("Disabling GPIO EM4 wake-up...\r\n");

    // Disable EM4 wake-up for the pin by clearing the bit in EM4WUEN
    GPIO->EM4WUEN &= ~(GPIO_IEN_EM4WUIEN4);  // Disable wake-up on PB03 (bit 6)
    GPIO_IntDisable(1 << INTERRUPT_PIN);
}

void GPIO_ODD_IRQHandler(void) {
    printf("GPIO_ODD_IRQHandler() ENTER\r\n");  // Corrected

    printf("GPIO_INT pin [%d] \r\n", GPIO_PinOutGet(INTERRUPT_PORT, INTERRUPT_PIN));

    if (isFirstIteration) {
        isFirstIteration = false;  // Skip handling in the first iteration
    }
    else {
        // Clear the interrupt for subsequent iterations
        GPIO_IntClear(1 << INTERRUPT_PIN);

        // Re-enable the interrupt for the specified pin
        GPIO_IntDisable(1 << INTERRUPT_PIN);

        // Re-enable BURTC wake-up mechanism
        NVIC_EnableIRQ(BURTC_IRQn);
        burtc_irq_period = 5000;
        change_burtc_compare_value(burtc_irq_period);

        // Reset wakeup counter for next 15 BURTC cycles
        em4_wakeup_counter = 0;
        set_em2_mode();
        em_mode_2 = true;
        is_capacitor_voltage_enough = true;
    }
    printf("GPIO_ODD_IRQHandler() EXIT\r\n");
}
  ...
  if(em4_wakeup_counter >= 14)
    {
      ...
      em4_wakeup_counter = 0; // reset EM4 wakeup counter
      BURAM->RET[0].REG = em4_wakeup_counter;
      disable_GPIO_EM4Wakeup();

      BURTC_IntEnable(BURTC_IEN_COMP);    // Enable the compare interrupt
      NVIC_EnableIRQ(BURTC_IRQn);
      BURTC_Enable(true);
      printf("--Burtc re-enabled \r\n");
    }

  // Check RESETCAUSE, update and print EM4 wakeup count
  checkResetCause();

  //GPIO_PinModeSet(COMP_VPOS_PORT, COMP_VPOS_PIN, gpioModePushPull, 0);
  //GPIO_PinOutSet(COMP_VPOS_PORT, COMP_VPOS_PIN);
  DEBUG_PRINT_MAX("AppInt -> PC3_Presence: %d, SDA: %d, SCL: %d, PB4_COMP_Vpos: %d\r\n",
             GPIO_PinOutGet(PRESENCE_DETECTOR_PORT, PRESENCE_DETECTOR_PIN),
             GPIO_PinOutGet(I2C_SCL_PORT, I2C_SCL_PIN),
             GPIO_PinOutGet(I2C_SDA_PORT, I2C_SDA_PIN),
             GPIO_PinOutGet(COMP_VPOS_PORT, COMP_VPOS_PIN));
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_soft_timer_id:
      ...
      if(adv_presence)
      {
        DEBUG_PRINT_MAX("lazy_1_Timer (adv_presence) -> PC3_Presence: %d, SDA: %d, SCL: %d, "
                "PB4_COMP_Vpos: %d\r\n",
                GPIO_PinOutGet(PRESENCE_DETECTOR_PORT, PRESENCE_DETECTOR_PIN),
                GPIO_PinOutGet(I2C_SCL_PORT, I2C_SCL_PIN),
                GPIO_PinOutGet(I2C_SDA_PORT, I2C_SDA_PIN),
      ...
      else if(enter_EM4)
      {
        ...
        app_assert_status(sc); */
        DEBUG_PRINT_MAX("lazy_2_Timer (enter_EM4) -> PC3_Presence: %d, SDA: %d, SCL: %d, "
                "PB4_COMP_Vpos: %d, em4_wakeup_counter: %d\r\n\r\n",
                GPIO_PinOutGet(PRESENCE_DETECTOR_PORT, PRESENCE_DETECTOR_PIN),
                GPIO_PinOutGet(I2C_SCL_PORT, I2C_SCL_PIN),
                GPIO_PinOutGet(I2C_SDA_PORT, I2C_SDA_PIN),
                GPIO_PinOutGet(COMP_VPOS_PORT, COMP_VPOS_PIN), em4_wakeup_counter);

        clear_em2_mode();
        enter_EM4 = false;

        // If the wake count reaches 14, enter EM4
        if (em4_wakeup_counter >= 14) {
            disableBURTCInterrupt(); // Stop waking-up from EM4 via on BURTC
            initGPIO_INT(); //configure EM4 wake-up via GPIO interrupt
        }
