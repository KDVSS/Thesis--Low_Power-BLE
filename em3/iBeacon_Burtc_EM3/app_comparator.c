...
const double voltageDividerRatio = 1; // Set to 1.0 if no external voltage divider is used
// A constant for the reference voltage in volts
const double referenceVoltageV = 3.300;
// A constant for the reference voltage in milli volts
const double referenceVoltageMV = 3300;
...
const double calibrationFactor = 1; // When PV cell is connected with lower voltages below 3.99V
// Previously received super capacitor voltage
double previous_superCapVoltage = 0;
...
void my_IADC_disable(IADC_TypeDef *iadc)
{
  if (trigger_IADC_Conversions == true) {
...
{
  printf("initGPIO_INT() ENTER\r\n");

  //GPIO_PinModeSet(INTERRUPT_PORT, INTERRUPT_PIN, gpioModePushPull, 0); // Pull-up enabled
  GPIO_PinModeSet(INTERRUPT_PORT, INTERRUPT_PIN, gpioModeInput, 0); // Pull-up enabled

  // Configure interrupt for rising edge on PB4 (V_OUT goes high)
  GPIO_ExtIntConfig(INTERRUPT_PORT, INTERRUPT_PIN, INTERRUPT_PIN, true, false, true); // Enable rising-edge interrupt on PB3

  // Clear any pending GPIO interrupt in NVIC
  NVIC_ClearPendingIRQ(GPIO_ODD_IRQn);
  NVIC_EnableIRQ(GPIO_ODD_IRQn );

  printf("initGPIO_INT() EXIT\r\n");
}
...
void GPIO_ODD_IRQHandler(void) {
    ...
    if (isFirstIteration) {
        ...
        // Disable the GPIO interrupt to prevent further wake-ups
...
void app_init(void)
{
  ...
  // Set priority for BURTC interrupt (0 = highest priority)
  //NVIC_SetPriority(BURTC_IRQn, 3);
  initBURTC();

  // Set priority for GPIO_EVEN interrupt (higher number = lower priority)
    ...
    case sl_bt_evt_system_soft_timer_id:
      ...
      else if(enter_EM3)
      {
        ...
        printf("lazy_2_Timer (enter_EM3) -> PC3_Presence: %d, SDA: %d, SCL: %d, "
               "PB4_COMP_Vpos: %d, em3_wakeup_counter: %d\r\n\r\n",
                   GPIO_PinOutGet(PRESENCE_DETECTOR_PORT, PRESENCE_DETECTOR_PIN),
                   GPIO_PinOutGet(I2C_SCL_PORT, I2C_SCL_PIN),
                   GPIO_PinOutGet(I2C_SDA_PORT, I2C_SDA_PIN),
                   GPIO_PinOutGet(COMP_VPOS_PORT, COMP_VPOS_PIN), em3_wakeup_counter);

        // If the wake count reaches 14, enter EM3
        if (em3_wakeup_counter >= 14) {
            // Disable BURTC_IRQn to stop waking on BURTC
            ...
            //printf("Disabled Burtc and re-enabled INT");
        }
      }
      break;
